...
import { MistakesSidebar } from './MistakesSidebar';
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (surahLimits.length > 0) {
        const { scrollTop, clientHeight } = wrapper as HTMLElement;
    ...
    <div className={`app-layout ${!isHeaderExpanded ? 'mobile-collapsed' : ''}`}>
      <header className={`app-header ${!isHeaderExpanded ? 'hidden-mobile' : ''}`}>
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          ...
          <SettingsButton
            onMushafTypeChange={(t) => { setMushafType(t); setCurrentPage(1); }}
...
export default AnnotationMushaf;
